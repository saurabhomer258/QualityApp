package com.example.saurabhomer.qualityapp.SkuCheckReport;

public class SkuCheckReport100Model {
    private String edt_readyquantity;
    private String edt_checkquantity;
    private String edt_size;
    private String countryhasbeencheck;
    private String labelhasbeencheck;
    private String barcodehasbeencheck;
    private String colorhasbeencheck;
    private String polybaghasbeencheck;
    private String polystikerhasbeencheck;
    private String sizetaghasbeencheck;
    private String hangerhasbeencheck;
    private String hagertaghasbeencheck;
    private String otherhasbeencheck;
    private String packingmethodhasbeencheck;
    private String sizestickerhasbeencheck;
    private String next;
    private String done;

    public String getEdt_readyquantity() {
        return edt_readyquantity;
    }

    public void setEdt_readyquantity(String edt_readyquantity) {
        this.edt_readyquantity = edt_readyquantity;
    }

    public String getEdt_checkquantity() {
        return edt_checkquantity;
    }

    public void setEdt_checkquantity(String edt_checkquantity) {
        this.edt_checkquantity = edt_checkquantity;
    }

    public String getEdt_size() {
        return edt_size;
    }

    public void setEdt_size(String edt_size) {
        this.edt_size = edt_size;
    }

    public String getCountryhasbeencheck() {
        return countryhasbeencheck;
    }

    public void setCountryhasbeencheck(String countryhasbeencheck) {
        this.countryhasbeencheck = countryhasbeencheck;
    }

    public String getLabelhasbeencheck() {
        return labelhasbeencheck;
    }

    public void setLabelhasbeencheck(String labelhasbeencheck) {
        this.labelhasbeencheck = labelhasbeencheck;
    }

    public String getBarcodehasbeencheck() {
        return barcodehasbeencheck;
    }

    public void setBarcodehasbeencheck(String barcodehasbeencheck) {
        this.barcodehasbeencheck = barcodehasbeencheck;
    }

    public String getColorhasbeencheck() {
        return colorhasbeencheck;
    }

    public void setColorhasbeencheck(String colorhasbeencheck) {
        this.colorhasbeencheck = colorhasbeencheck;
    }

    public String getPolybaghasbeencheck() {
        return polybaghasbeencheck;
    }

    public void setPolybaghasbeencheck(String polybaghasbeencheck)
    {
        this.polybaghasbeencheck = polybaghasbeencheck;
    }

    public String getPolystikerhasbeencheck() {
        return polystikerhasbeencheck;
    }

    public void setPolystikerhasbeencheck(String polystikerhasbeencheck)
    {
        this.polystikerhasbeencheck = polystikerhasbeencheck;
    }

    public String getSizetaghasbeencheck() {
        return sizetaghasbeencheck;
    }

    public void setSizetaghasbeencheck(String sizetaghasbeencheck) {
        this.sizetaghasbeencheck = sizetaghasbeencheck;
    }

    public String getHangerhasbeencheck() {
        return hangerhasbeencheck;
    }

    public void setHangerhasbeencheck(String hangerhasbeencheck) {
        this.hangerhasbeencheck = hangerhasbeencheck;
    }

    public String getHagertaghasbeencheck() {
        return hagertaghasbeencheck;
    }

    public void setHagertaghasbeencheck(String hagertaghasbeencheck) {
        this.hagertaghasbeencheck = hagertaghasbeencheck;
    }

    public String getOtherhasbeencheck() {
        return otherhasbeencheck;
    }

    public void setOtherhasbeencheck(String otherhasbeencheck) {
        this.otherhasbeencheck = otherhasbeencheck;
    }

    public String getPackingmethodhasbeencheck() {
        return packingmethodhasbeencheck;
    }

    public void setPackingmethodhasbeencheck(String packingmethodhasbeencheck) {
        this.packingmethodhasbeencheck = packingmethodhasbeencheck;
    }

    public String getSizestickerhasbeencheck() {
        return sizestickerhasbeencheck;
    }

    public void setSizestickerhasbeencheck(String sizestickerhasbeencheck) {
        this.sizestickerhasbeencheck = sizestickerhasbeencheck;
    }

    public String getNext() {
        return next;
    }

    public void setNext(String next) {
        this.next = next;
    }

    public String getDone() {
        return done;
    }

    public void setDone(String done) {
        this.done = done;
    }
}
