...
public class DailyfinishingOutsideAdmin extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FirebaseDatabase.getInstance().getReference("dailyFinishingoutside")
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                strdate=intent.getStringExtra("outsidedate");
                strfinish = intent.getStringExtra("outsidefinishing");
                ...
                for (int i =0 ;i< dailyFinishinfModels.size() ; i++) {
                    ...
                    if (insidedate1.equals(strdate) && finishingline.equals(strfinish)) {
                        ...
                        if (i == dailyFinishinfModels.size() - 1) {
                            Toast.makeText(DailyfinishingOutsideAdmin.this, "Opps! No data found", Toast.LENGTH_SHORT).show();
                            finish();
                        }
                    }
                }
    ...
    private void setToal(ArrayList<DialyFinishingAnalysisModel> DAILYFINIFSHINGMODELLIST) {
        ...
        for(int i=1;i<DAILYFINIFSHINGMODELLIST.size();i++){
            ...
            items.setTotalCheck(items.getTotalCheck() + dialyFinishingAnalysisModel1.getTotalCheck());
        }
        ...
        FirebaseDatabase.getInstance().getReference("dailyFinishingoutside")
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if (mainDailyFinishingModel != null) {
                    ArrayList<DailyFinishinfModels> dialyFinishingAnalysisModels11 = mainDailyFinishingModel.getDailyFinishingModels();

                    for (int j = 0; j < mainDailyFinishingModel.getDailyFinishingModels().size(); j++) {
                        DailyFinishinfModels dailyFinishinfModels = mainDailyFinishingModel.getDailyFinishingModels().get(j);
                        String insidedate1 = dailyFinishinfModels.getDate();
                        String finishingline = dailyFinishinfModels.getFinishingLine();
                        Log.d(insidedate1, "check");
                        Log.d(strdate, "check sys");
                        if (insidedate1.equals(strdate) && finishingline.equals(strfinish)) {
                            ArrayList<DialyFinishingAnalysisModel> dialyFinishingAnalysisModels = mainDailyFinishingModel.getDailyFinishingModels().get(j).getDialyFinishingAnalysisModels();
